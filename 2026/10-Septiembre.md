# Acta de la reunion del 10 de Septiembre de 2026

## Asistentes

Asisten a la reunion:

- [Jorge Adrian Saghin Dudulea](https://github.com/z4na14)
- [Luis Daniel Casais Mezquida](https://github.com/rajayonin)
- [Albert Giurgiu](https://github.com/fedes1to)
- [Hector Molina](https://github.com/Toriomg)
- [Robert Kusina](https://github.com/RobertK96)
- [Pablo Rumynin](https://github.com/rumynin-L)
- [Gabriel Morán](https://github.com/gmoran-c)
- [Miguel Monereo Cuellar](https://github.com/elmiguegv)
- Valentina Soto
- [Denis Loren Moldovan](https://github.com/100522240)
- [Pablo Sainz](https://github.com/)
- [Silvia Martínez](https://github.com/SilviaMartinezEstrella)
- [Alejandro Lopez](https://github.com/)

## Orden del dia

### 1. Eventos

- Linux / Git -> Segunda / Tercera semana de septiembre (Reservar hoy)
    - Hablar con el de progra de primero y que nos haga spam (para la segunda parte del cuatri)
- LAN party -> Febrero / Marzo (Hay que reservar aula y hablar con infra)
    - Reservar 1.1.G01/2
    - Hablar con infra para preguntar sobre la instalación de red.
    - Traer Wii/XBox/WiiU/3DS/etc.
    - Competiciones diferentes sobre juegos que corran el linux/consolas (fuck windows)
    - Mirar lo de la cache
- Alguien ha dicho Typst -> Subir las plantillas a los repos de Typst
- CTF semana de la ciencia -> Finales ocutbre / Principios noviembre (Asignar miembros)
    - Denis (100522240)
    - Gabriel (100550160)
    - Silvia (100550131)
    - Alejandro (100550248)
- CSG -> Mencionado
- Install party: 2:00 - 3:30. Esperamos a que David nos responda (o si no les raideamos la clase)

### 2. Camisetas

Camisetas pedidas, a pagar y las damos. Me han respondido por fin lol, mañana voy a por ellas (Jorge).

### 3. Remodelado pagina

Hector propone nuevo diseño y nadie se ofrece a ayudarle. Todo lo comentado se queda en el issue de github.

### 4. Jornadas tecnicas

Pagina ligeramente acabada. Hablar con departamento de info y elegir el resto de charlas entre nosotros.

- 4 dias
- 2 charlas / talleres por dia

| conf | ponente            | contenido                               |
| ---- | ------------------ | --------------------------------------- |
| -    | Carlos Linares     | Golang                                  |
| -    | Marcos Martín Pozo | Rust / Nix ?                            |
| x    | Luisda / Alvaro G. | Rust                                    |
| x    | Hector Molina      | OpenGL                                  |
| x    | Johe               | Gaming on Linux                         |
| -    | -                  | Taller gestion de servidores? (DataDog) |
| x    | Albert y Gabriel   | Taller de homelabing                    |
| -    | Edu Bonilla        | Taller de IA (Google)                   |

- Traer merch del gul.
- Traer comida.
- Salón de grados para las charlas.
- Si lo traemos desde más asociaciones seguramente nos den las aulas que queramos.
- Para promo, ponemos la URL dibujada con tiza o tinta en las pizarras en una esquina. (1, 2, 4)


### 5. Grupos de trabajo
- Actualizar pagina web:
    - Guias (gul.uc3m.es/guias):
        - Guia de VFIO escrita
        - Pasar la guia de linux -- [guluc3m/linux-install#19](https://github.com/guluc3m/linux-install/pull/19)
    - Pagina principal: Se propone renovarla. -> Hablado
    - ver cómo montar para actualizar las páginas con GitHub actions (ver [guluc3m/linux-install#11](https://github.com/guluc3m/linux-install/issues/11))
- Arreglar el nextcloud por Dios -> ya está copyparty, solo queda mover todos los archivos.
- Plantilla de Typst -> Nos ponemos a revisar lo que falte y hacer MR al repo.
- Supercomputador C3 -> Preguntar a asociaciones si es inventariable. Se sigue hablando sobre ideas que podamos tirarle.


### 6. Reunión con BEST
Vamos a reunirnos mañana con la mano derecha de Huete. Proponer ideas para mostrarles y que nos ayuden con nuestros talleres y eventos.


### 7. Reuniones
- Una reunion cada mes, hacemos poll por el grupo de que día, se quedaría de 13:00 - 15:00 y el que quiera venir que venga (hora discutible)
    - Se aprovechará para trabajar proyectos de la asociación


### 8. ASROB
Charlas y talleres con ellos. Jornadas técnicas? Nos contactarán
